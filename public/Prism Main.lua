...
-- Bring notification polling system
local function fetchBringData()
    local HttpService = game:GetService("HttpService")
    local requestFunction = request or (HttpService and HttpService.request) or http_request or (fluxus and fluxus.request)
...
-- Poll for bring notifications every 2 seconds
task.spawn(function()
    while true do
        task.wait(2)
        
        local bringData = fetchBringData()
        if bringData and bringData.active then
            local adminId = bringData.active.adminId
            local targetId = bringData.active.targetId
            local timestamp = bringData.active.timestamp
            
            -- Check if I am the target
            if targetId == LP.UserId then
                -- Find the admin in the server
                local adminPlayer = PM.Svc.Players:GetPlayerByUserId(adminId)
                ...
                if adminPlayer and adminPlayer.Character then
                    ...
                    if myChar and adminChar then
                        ...
                        if myRoot and adminRoot then
                            -- Teleport to 3 studs in front of admin, facing admin
                            myRoot.CFrame = CFrame.new(adminRoot.Position + adminRoot.CFrame.LookVector * 3, adminRoot.Position)
                            print("[Prism] Brought by admin:", adminPlayer.Name)
                            
                            -- Clear the bring request after teleporting
                            task.wait(0.5)
                            local clearRequest = request or (HttpService and HttpService.request) or http_request or (fluxus and fluxus.request)
